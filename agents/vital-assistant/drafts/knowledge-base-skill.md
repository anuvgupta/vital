...
description: Answer questions about Vital synthesizer capabilities, parameters, and sound design techniques. Use when the user asks what parameters are available, how to achieve certain sounds, what specific knobs/settings do, or needs guidance on Vital's features without requesting a preset file.
---
...
2. **Explain relationships**: Help users understand how parameters interact (e.g., filter cutoff + resonance + drive).
3. **Suggest approaches**: When users ask "how do I make X sound," explain which parameters and ranges typically achieve that result — but don't output JSON unless they explicitly ask for a preset.
4. **Clarify limitations**: If something isn't possible in Vital (e.g., a feature that doesn't exist), say so clearly.
5. **Use correct terminology**: Match Vital's actual parameter names (e.g., `osc_1_spectral_morph_type`, not "spectral morph mode").

## Example Q&A patterns

**User**: "What filter types does Vital have?"
→ List the filter models (Analog, Dirty, Ladder, Digital, Diode, Formant, Comb, Phaser) with brief descriptions.

**User**: "How do I get a supersaw sound?"
→ Explain unison voices, detune, stack styles, and stereo spread — with typical values.

**User**: "What's the difference between spectral morph and distortion?"
→ Explain that distortion operates in the time domain (Sync, FM, RM, etc.) while spectral morph operates in the frequency domain (Vocode, Smear, etc.).

**User**: "Can I route an LFO to the reverb mix?"
→ Check the schema's modulation destinations and confirm yes/no with the exact parameter name.
