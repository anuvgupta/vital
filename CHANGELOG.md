...
- We improved API response handling to support mixed text+JSON responses: the response parser extracts JSON from markdown code fences (``` blocks), applies preset diffs from the JSON, and displays surrounding text as a chat message. This allows the AI to explain changes alongside applying them. Also added random whimsical completion phrases (`getCompletionPhrase()`) as fallback when no text accompanies a preset update.
