...
- **`truncateHistoryTo()` is one-way (shrink only) — cannot restore truncated history**:
    - When cancel-edit needed to restore conversation history after a truncation, the code saved only the history size (an integer) and relied on `truncateHistoryTo(saved_size)` to "undo". But `truncateHistoryTo()` only shrinks — it permanently deletes entries. Saving just the size is useless if the entries are already gone.
    - **Root cause**: `EditModeSnapshot` stored `saved_api_history_size` (int) instead of the actual history entries. After `truncateHistoryTo()` ran during restore, the entries were permanently lost; cancel could not grow the vector back to the saved size.
