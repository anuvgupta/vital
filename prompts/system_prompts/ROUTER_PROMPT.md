...
- Using 2 actions is preferable, and 3 if necessary. Maximum 4 actions, only for larger requests. never exceed 4 actions.
- If user expresses intent to save the sound, set save_required=true.
...
TECHNICAL vs NON-TECHNICAL — a request is technical if it references specific synth components or parameters:
- Oscillators (osc 1/2/3): waveform, transpose, tune, level, unison, detune, phase, distortion, spectral morph, destination
- Filters (filter 1/2/fx): model, cutoff, resonance, drive, mix, routing
- Envelopes (env 1-6), LFOs (lfo 1-8), Random LFOs (random 1-4): any timing or shape params
- Effects: chorus, compressor, delay, distortion, EQ, flanger, phaser, reverb
- Modulation connections, macros, wavetables, sample player
- Global: volume, polyphony, portamento, velocity tracking

If the user describes a sound by vibe, texture, genre, or instrument name without specifying parameters, the request is NON-TECHNICAL so set sound_design_required=true.
